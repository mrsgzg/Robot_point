...
def extract_camera_image(model, data, camera_id, width=640, height=480):
    ...
    Returns:
        numpy.ndarray: RGB image as uint8 array
    """
...
class SimulationVisualizer:
    def __init__(self, model, data, controller, collect_data=False, output_dir="collected_data"):
        ...
        self.camera_fov = 60  # Default FOV
        if collect_data:
            # Import only if needed
            from data_collection import DataCollector
            self.data_collector = DataCollector(model, data, self.camera.fixedcamid, output_dir)
            print(f"Data collection enabled. Saving to {output_dir}")
        
        # Add key to toggle data collection
    ...
    def set_camera_fov(self, fov):
        ...
        for i in range(self.model.ncam):
            if self.model.cam(i).name == "table_cam":
                self.model.cam(i).fovy = fov
                print(f"Set camera FOV to {fov} degrees")
                break
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        if key == glfw.KEY_ESCAPE and action == glfw.PRESS:
            ...
            self.camera_fov = max(10, self.camera_fov - 5)
            self.set_camera_fov(self.camera_fov)
        elif key == glfw.KEY_G and action == glfw.PRESS:
            # Increase FOV (zoom out)
            self.camera_fov = min(120, self.camera_fov + 5)
            self.set_camera_fov(self.camera_fov)
    ...
    def visualize_pointing_sequence(self):
        ...
        while not glfw.window_should_close(self.window):
            ...
            if self.collect_data and self.recording:
                try:
                    # Create camera image
                    ...
                    self.data_collector.collect_frame(
                        ...
                        counted_balls=self.controller.counted_balls,
                        motion_phase=self.motion_phase,  # Add motion phase
                        image=camera_img
                    )
                except Exception as e:
            ...
            try:
                ...
                if self.collect_data and self.recording:
                    recording_text = "RECORDING DATA [Press R to stop]"
                    mujoco.mjr_overlay(mujoco.mjtFontScale.mjFONTSCALE_150, 
                                     mujoco.mjtGridPos.mjGRID_TOPRIGHT, 
                                     viewport, recording_text, "", context)
                elif self.collect_data and not self.recording:
                    recording_text = "Press R to start recording"
                    mujoco.mjr_overlay(mujoco.mjtFontScale.mjFONTSCALE_150, 
                                     mujoco.mjtGridPos.mjGRID_TOPRIGHT, 
                                     viewport, recording_text, "", context)
        ...
        # Finalize data collection if needed
        if self.collect_data:
            self.data_collector.finish()
